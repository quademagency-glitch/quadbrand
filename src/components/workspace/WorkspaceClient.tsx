...
export default function WorkspaceClient({ workspace, members }: { workspace: Workspace; members: Member[] }) {
        ...
        <div className="md:col-span-2 space-y-6">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-4">
                      <div className="w-16 h-16 rounded-xl bg-[var(--bg-secondary)] border border-[var(--border)] flex items-center justify-center overflow-hidden">
                        {workspace.logo_url ? (
          ...
          {activeTab === "Members" && (
            ...
            >
              ...
              <div className="space-y-4">
                {members.map((m) => (
                  <div key={m.id} className="flex items-center justify-between p-4 rounded-xl border border-[var(--border)] bg-[var(--bg-secondary)]">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-full bg-[var(--accent-cyan)]/20 text-[var(--accent-cyan)] flex items-center justify-center font-bold uppercase overflow-hidden">
                        {m.avatar_url ? (
                      ...
                      <div>
                        <p className="text-sm font-semibold">{m.full_name || "You"}</p>
                        <p className="text-xs text-[var(--text-tertiary)]">{m.email}</p>
                      </div>
                    </div>
                    <span className="text-xs font-medium px-2 py-1 bg-[var(--bg-card)] rounded-md border border-[var(--border)] capitalize">
                      {m.role}
                    </span>
                  </div>
                ))}
          ...
          {activeTab === "API Keys" && (
            ...
            >
              ...
              <div className="text-center py-8 border border-dashed border-[var(--border)] rounded-xl">
                ...
                <p className="text-sm font-medium mb-4">API Keys & Webhooks are managed in Developer Settings</p>
                <a href="/workspace/developer" className="btn-gradient text-sm !py-2 !px-4 inline-flex items-center gap-2">
                  <Settings className="w-4 h-4" /> Go to Developer Settings
                </a>
              </div>
            </motion.div>
          )}
