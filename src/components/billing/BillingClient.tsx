...
import { CreditCard, Check, Zap, Sparkles, Loader2 } from "lucide-react";
...
export default function BillingClient({
  ...
  const handleBuyPack = async (packId: string, credits: number) => {
    setLoadingPack(credits);
    try {
      // In this system, packId maps to a planId in payments.ts 
      // For credit packs, we might need to adjust payments.ts or just pass the amount
      const res = await fetch("/api/billing/checkout", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ 
          planId: packId, // Note: You'll need to define 'pack_100' etc in payments.ts!
  ...
  const planName = workspace.plan ? workspace.plan.charAt(0).toUpperCase() + workspace.plan.slice(1) : "Free Plan";
  const planCredits = workspace.plan === "pro" ? 800 : workspace.plan === "starter" ? 200 : 20;
        ...
        >
          <div className="flex items-start justify-between mb-6">
            <div>
              ...
              <p className="text-sm text-[var(--text-secondary)]">
                Includes top-ups and monthly allowance
              </p>
            </div>
          ...
          <div className="mb-8">
            <div className="flex items-end gap-2 mb-2">
              <span className="text-4xl font-bold" style={{ fontFamily: "var(--font-outfit)" }}>{workspace.credits_pool}</span>
              <span className="text-[var(--text-secondary)] pb-1">credits remaining</span>
            </div>
            {/* Progress bar */}
            <div className="h-3 bg-[var(--bg-secondary)] rounded-full overflow-hidden border border-[var(--border)] relative">
              {workspace.credits_pool === 0 ? (
                <div 
        ...
        >
          <div className="mb-6">
            ...
            <div className="flex items-baseline gap-1 mt-2">
              <span className="text-3xl font-bold" style={{ fontFamily: "var(--font-outfit)" }}>{workspace.plan === "pro" ? "$29" : workspace.plan === "starter" ? "$9" : "$0"}</span>
      ...
      >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {[
            ...
            { id: "pack_500", credits: 500, price: "$20", popular: true },
            ...
            >
              ...
              <button 
                onClick={() => handleBuyPack(pack.id, pack.credits)}
                disabled={loadingPack !== null}
                className="w-full py-2 rounded-lg text-sm font-medium border border-[var(--border)] bg-[var(--bg-card)] hover:bg-[var(--bg-secondary)] transition-colors flex items-center justify-center gap-2"
              >
                {loadingPack === pack.credits ? <Loader2 className="w-4 h-4 animate-spin" /> : null}
                Buy Pack
              </button>
            </div>
          ))}
