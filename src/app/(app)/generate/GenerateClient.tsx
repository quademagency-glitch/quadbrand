...
export default function GenerateClient({
  ...
  const [selectedBrand, setSelectedBrand] = useState<string>(brands.length > 0 ? brands[0].id : "");
  ...
  const handleGenerate = async () => {
    ...
    if (credits < currentModel.credits * selectedRatios.length * numOutputs) {
    ...
    try {
      ...
      // Collect all image URLs from all variants across all ratios
      const urls: string[] = [];
      results.forEach(r => {
        if (r.results) {
          r.results.forEach((res: any) => urls.push(res.imageUrl));
        } else if (r.imageUrl) {
        ...
        >
            ...
            <div className="relative">
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <span>
                    {brands.find(b => b.id === selectedBrand)?.name || "No Brand Selected"}
                  </span>
                </div>
              ...
              <AnimatePresence>
                {brandOpen && (
                  <motion.div
                    ...
                    animate={{ opacity: 1, y: 0 }}
                    exit={{ opacity: 0, y: -10 }}
                    className="absolute top-full left-0 right-0 mt-2 bg-[var(--bg-elevated)] border border-[var(--border)] rounded-xl shadow-lg z-50 py-2"
                  >
                    {brands.length === 0 ? (
                      <div className="px-4 py-2 text-sm text-[var(--text-tertiary)]">No brands available</div>
                    ) : (
                      brands.map(brand => (
                        <button
                          key={brand.id}
                        ...
                        >
                          {brand.name}
                          {selectedBrand === brand.id && <Check className="w-4 h-4 text-[var(--accent-cyan)]" />}
                        </button>
                      ))
                    )}
                  </motion.div>
          ...
          <div>
              ...
              {[1, 2, 3, 4].map((num) => (
                <button
                  ...
                  className={`flex-1 py-2 rounded-lg border text-sm transition-all ${
                    numOutputs === num
                      ? "border-[var(--accent-cyan)] bg-[var(--accent-cyan-light)] text-[var(--accent-cyan-dark)] font-semibold"
                      : "border-[var(--border)] bg-[var(--bg-card)] text-[var(--text-secondary)] hover:border-[var(--border-hover)]"
                  }`}
                >
                  {num}
            ...
            <p className="text-xs text-[var(--text-tertiary)] mt-2">
              Cost: {currentModel.credits * numOutputs * selectedRatios.length} credits total
            </p>
          </div>
          ...
          >
            <div className="relative z-10 flex items-center justify-center gap-2">
              ...
              <span>
                {generating
                  ? `Generating ${selectedRatios.length * numOutputs > 1 ? `${selectedRatios.length * numOutputs} images` : "image"}...`
                  : `Generate — ${currentModel.credits * selectedRatios.length * numOutputs} credits`}
              </span>
            </div>
